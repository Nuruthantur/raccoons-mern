...
const signup = async(req, res) => {
  ...
  if (!email || !password) return res.status(400).json({error: "No fields must be empty"})
  try {
    ...
    console.log(error);
    if (error.code === 11000) res.status(400).json({ error: "Email already registered!" })
...
const login = async(req, res) => {
  ...
  try {
    ...
    if (!foundUser) return res.status(404).json({ error: "No user with that email!" });
